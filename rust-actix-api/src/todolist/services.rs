...
use crate::{ AppState, TodolistEntry };    //project-folder/main.rs
use super::models::{ CreateEntryData, UpdateEntryData };  //models.rs

#[get("/todolist/entries")]
async fn get_entries(data: web::Data<AppState>) -> impl Responder {
    HttpResponse::Ok().json(
        data.todolist_entries.lock().unwrap().to_vec()  //set to vec/array the AppState todolist_entries
    )
}
